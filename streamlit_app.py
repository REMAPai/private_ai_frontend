...
def get_db_connection():
    ...
    if not db_exists:
        ...
        if current_time - st.session_state.db_last_check >= retry_delay:
            ...
            if st.session_state.db_retry_count < max_retries:
                ...
                with st.spinner(f"Waiting for database... (attempt {st.session_state.db_retry_count}/{max_retries})"):
                ...
                st.info(f"Open WebUI health check: {'✓ Healthy' if webui_healthy else '✗ Not responding'}")
    ...
    try:
        ...
        if not tables_exist:
            ...
            conn.close()
            st.warning(f"Database file exists at {db_path} but appears to be empty (no tables found).")
            st.info("Open WebUI is still initializing the database schema. Please wait a few moments.")
            st.info(f"Open WebUI health check: {'✓ Healthy' if webui_healthy else '✗ Not responding'}")
...
page = st.sidebar.selectbox(
    "Navigation",
    ["Database Tables", "Credit Groups", "User Assignments", "Model Management", "Base Settings", "Migrations"]
)
...
if page == "Database Tables":
            ...
            for idx, row in settings_df.iterrows():
                ...
                with st.expander(f"{key}: {value}"):
                    with st.form(f"edit_setting_{idx}"):
                        new_value = st.text_input("Value", value=str(value), key=f"val_{idx}")
                        new_desc = st.text_area("Description", value=str(desc), key=f"desc_{idx}", height=60)
                        ...
                        if submitted:
                            ...
                            result = execute_query(conn, update_query, (new_value, new_desc, key))
    ...
    with col2:
        ...
        with st.form("add_setting"):
            ...
            new_desc = st.text_area("Description", placeholder="Description of the setting")
            
            submitted = st.form_submit_button("Create Setting", use_container_width=True)
            if submitted:
                if new_key:
                    # Check if key exists
                    check_df = get_table_data(conn, "token_tracking_base_settings", where_clause=f"setting_key = '{new_key}'")
                    if check_df is not None and not check_df.empty:
                        ...
                        result = execute_query(conn, insert_query, (new_key, new_value, new_desc))
